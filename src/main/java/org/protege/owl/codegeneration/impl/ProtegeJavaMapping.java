...
public class ProtegeJavaMapping {
    ...
    public void add(String protegeClassName, Class<?> javaInterface,
    ...
    public void add(OWLClass protegeClass, Class<?> javaInterface,
            Class<? extends WrappedIndividualImpl> javaImplementation) {
    ...
    public <X> X create(Class<? extends X> javaInterface, String name) {
        ...
        ontology.getOWLOntologyManager().addAxiom(ontology,
                dataFactory.getOWLClassAssertionAxiom(cls, individual));
        return constructImplementation((Class<? extends X>) entry.getJavaImplementation(),
                individual.getIRI());
    }
    ...
    private <X> X constructImplementation(Class<? extends X> implType, IRI id) {
        try {
            Constructor<? extends X> con = implType
                    .getConstructor(new Class[] { CodeGenerationInference.class, IRI.class });
            return con.newInstance(new Object[] { inference, id });
        } catch (Throwable t) {
            ClassCastException classcast = new ClassCastException(
                    "Resource " + id.toString() + " could not be cast to type " + implType);
            classcast.initCause(t);
            throw classcast;
        }
    }

    private <X> Class<? extends X> getJavaImplementation(OWLNamedIndividual resource,
    ...
    private static class JavaAndOwlBean {
        ...
        public JavaAndOwlBean(OWLClass protegeClass, Class<?> javaInterface,
                Class<? extends WrappedIndividualImpl> javaImplementation) {
