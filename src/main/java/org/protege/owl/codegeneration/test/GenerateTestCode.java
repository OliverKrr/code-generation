...
/**
 * Ordinarily I wouldn't corrupt the main source tree with testing code. But
 * this seems like a reasonable exception at this time. This bootstraps the
 * maven test build process. It should be the only test file that appears in the
 * main tree.
 * 
 * @author tredmond
 *
...
public class GenerateTestCode {
    ...
    static {
        ...
        ONTOLOGY_ROOT = aggregator_ontology_root.exists() ? aggregator_ontology_root
    ...
    private static void generateSimpleJavaCode() throws OWLOntologyCreationException, InstantiationException,
            IllegalAccessException, ClassNotFoundException, IOException {
        File outputFolder = getOutputFolder();
        Utilities.deleteFolder(outputFolder);
        generateSimpleJavaCode("CodeGeneration001.owl", "inferred.testSimple", "MyInferredFactory", true,
                outputFolder);
        generateSimpleJavaCode("CodeGeneration001.owl", "std.testSimple02", "MySimpleStdFactory", false,
                outputFolder);
        generateSimpleJavaCode("CodeGeneration002.owl", "inferred.propertyValues",
                "InferredPropertyValuesFactory", true, outputFolder);
        generateSimpleJavaCode("CodeGeneration003.owl", "inferred.generate03", "InferredGenerate03Factory",
                true, outputFolder);
        generateSimpleJavaCode("CodeGeneration004.owl", "inferred.generate04", "InferredGenerate04Factory",
                true, outputFolder);
        generateSimpleJavaCode("pizza.owl", "inferred.pizza", "MyInferredPizzaFactory", true, outputFolder);
        generateSimpleJavaCode(GenerateTestCode.FEB_TBOX_ONTOLOGY, "inferred.febissue", "FebIssueFactory",
                true, outputFolder);
    ...
    private static void generateSimpleJavaCode(String ontologyLocation, String packageName,
            String factoryName, boolean useInference, File outputFolder) throws OWLOntologyCreationException,
        ...
        OWLOntology owlOntology = manager
                .loadOntologyFromOntologyDocument(new File(GenerateTestCode.ONTOLOGY_ROOT, ontologyLocation));
        ...
        if (useInference) {
            // OWLReasonerFactory rFactory = (OWLReasonerFactory)
            // Class.forName("uk.ac.manchester.cs.jfact.JFactFactory").newInstance();
            OWLReasonerFactory rFactory = (OWLReasonerFactory) Class
                    .forName("org.semanticweb.HermiT.Reasoner$ReasonerFactory").newInstance();
            // OWLReasonerFactory rFactory = (OWLReasonerFactory)
            // Class.forName("com.clarkparsia.pellet.owlapiv3.PelletReasonerFactory").newInstance();
        ...
        LOGGER.info("Generating source code for ontology " + ontologyLocation + " ("
                + (useInference ? "inferred - " : "asserted -") + (System.currentTimeMillis() - startTime)
                + "ms).");
    }
    ...
    public static void generateCustomJavaCode() throws IOException, OWLOntologyCreationException {
        ...
        OWLOntologyManager manager = OWLManager.createOWLOntologyManager();
        OWLOntology owlOntology = manager.loadOntologyFromOntologyDocument(
                new File(GenerateTestCode.ONTOLOGY_ROOT, "CustomNames.owl"));
        CodeGenerationOptions options = new CodeGenerationOptions();
        options.setPackage(fullPackageName);
    ...
    public static void addIRIMappers(OWLOntologyManager manager) {
        manager.getIRIMappers().add(new SimpleIRIMapper(
                IRI.create("http://jamesnaish.wordpress.com/ROREKnowledgeModel.owl"),
                IRI.create(new File(ONTOLOGY_ROOT, FEB_PATH + File.separator + "ROREKnowledgeModel.owl"))));
        manager.getIRIMappers().add(new SimpleIRIMapper(
                IRI.create("http://jamesnaish.wordpress.com/Model/AutopilotSourceModel.owl"),
